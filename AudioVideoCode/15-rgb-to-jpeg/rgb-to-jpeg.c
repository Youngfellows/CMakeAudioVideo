...
int main()
{
    ...
    for (int y_index = 0; y_index < block_size; y_index++)
    {
        ...
        for (int i = 0; i < 64; i++)
        {
            y_blocks_dct[y_index][i] = y_block[i] - 128;
        }
        for (int i = 0; i < 64; i++)
        {
    ...
    for (int u_index = 0; u_index < block_size; u_index++)
    {
        ...
        for (int i = 0; i < 64; i++)
        {
            u_blocks_dct[u_index][i] = u_block[i] - 128;
        }
        for (int i = 0; i < 64; i++)
        {
    ...
    for (int v_index = 0; v_index < block_size; v_index++)
    {
        ...
        for (int i = 0; i < 64; i++)
        {
            v_blocks_dct[v_index][i] = v_block[i] - 128;
        }
        for (int i = 0; i < 64; i++)
        {
    ...
    phcac[0]->output = bs;
    huffman_encode_init(phcac[0], 1);
    phcac[1]->output = bs;
    huffman_encode_init(phcac[1], 1);
    phcdc[0]->output = bs;
    huffman_encode_init(phcdc[0], 1);
    phcdc[1]->output = bs;
    huffman_encode_init(phcdc[1], 1);
