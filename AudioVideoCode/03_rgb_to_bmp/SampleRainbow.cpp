...
int main(int arg, char *argv[])
{
    ...
    {
        uint32_t currentColor = RAINBOW_COLORS[0]; // 当前颜色，默认是红
        if (i < 100)
        {
            // std::cout << "main:: RAINBOW_COLORS[0]:" << RAINBOW_COLORS[0] << std::endl;
        ...
        uint32_t r = (currentColor & 0xFF0000) >> 16;
        // std::cout << "main: currentColor:" << currentColor << ",r:" << r << ",0xFF0000:" << 0xFF0000 << std::endl;

        // 获取当前颜色的 G 分量
        // uint8_t g = (currentColor & 0x00FF00) >> 8;
        uint32_t g = (currentColor & RGB24_MASK_GREEN) >> 8;

        // 获取当前颜色的 B 分量
        // uint8_t b = (currentColor & 0x0000FF);
        uint32_t b = (currentColor & RGB24_MASK_BLUE);
