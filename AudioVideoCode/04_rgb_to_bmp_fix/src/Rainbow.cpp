...
// 计算每一行像素 4 字节对齐后的字节数
int Rainbow::caculateLineBytes(int width)
{
    //******* 四字节对齐 *******
...
bool Rainbow::writeRainbow(const char *outputFile, int width, int height)
{
    ...
    fileHeader.bfSize = sizeof(BitmapFileHeader) + sizeof(BitmapInfoHeader) + lineBytes * height;
    ...
    for (int i = 0; i < width; i++)
    {
        ...
        for (int j = 0; j < height; ++j)
        {
            ...
            lineBytesArray[currentIndex + 1] = b;
