...
int test_libjpeg_turbo_decompress()
{
...
#else
    std::string image_path{"./jepg-resource/"};
    std::string res_image_path{"./resource/"};
#endif

    std::string image_name = image_path + "libjpeg-turbo-test-image.jpg";
    ...
    std::string result_image = res_image_path + "result_tirg.png";
    FILE *out_file = fopen(result_image.c_str(), "wb");
    if (!out_file)
    {
        fprintf(stderr, "open file fail: %s\n", result_image.c_str());
        return -1;
    }
    fwrite(data.get(), width * height * 3, 1, out_file);
    fclose(out_file);
    ...
    std::string result_image2 = res_image_path + "result_tirg2.png";
    out_file = fopen(result_image2.c_str(), "wb");
    if (!out_file)
    {
        fprintf(stderr, "open file fail: %s\n", result_image2.c_str());
        return -1;
...
int test_libjpeg_turbo_compress()
{
    ...
    long long t1 = Timer::getNowTime();
    get_jpeg_compress_data(p, width, height, 3, JCS_EXT_BGR, quality, &p, length, free_in_buffer);
    long long t2 = Timer::getNowTime();
    ...
    t1 = Timer::getNowTime();
    get_jpeg_compress_data2(p, width, height, TJPF_BGR, &p, &jpegSize, TJSAMP_420, quality, 0);
    t2 = Timer::getNowTime();
...
int test_libjpeg_turbo()
{
    ...
    unsigned long free_in_buffer;
    get_jpeg_compress_data(p1, width, height, 3, JCS_GRAYSCALE, quality, &p, length, free_in_buffer);
