...
{
  return call_method<jboolean>(m_object, "isFeatureSupported", "(Ljava/lang/String;)Z", jcast<jhstring>(name));
}

bool CJNIMediaCodecInfoCodecCapabilities::isFeatureRequired(const std::string& name)
{
  return call_method<jboolean>(m_object, "isFeatureRequired", "(Ljava/lang/String;)Z", jcast<jhstring>(name));
}
