  ...
  const std::string &cipherAlgorithm, const std::string &macAlgorithm) const
{
  return call_method<jhobject>(m_object,
      "getCryptoSession", "([BLjava/lang/String;Ljava/lang/String;)Landroid/media/MediaDrm$CryptoSession;",
        jcast<jhbyteArray>(sessionId), jcast<jhstring>(cipherAlgorithm), jcast<jhstring>(macAlgorithm));
}
...
{
  call_method<void>(m_object,
    "restoreKeys", "([B[B)V", jcast<jhbyteArray>(sessionId), jcast<jhbyteArray>(keySetId));
}
